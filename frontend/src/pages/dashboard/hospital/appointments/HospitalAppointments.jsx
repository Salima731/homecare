...
import { Calendar, Clock, Search, Filter, User, Stethoscope, CheckCircle, XCircle, AlertCircle, Loader } from 'lucide-react';
import { useGetHospitalAppointmentsQuery } from '../../../../features/appointments/appointmentApiSlice';
...
const HospitalAppointments = () => {
  ...
  const { data, isLoading } = useGetHospitalAppointmentsQuery(
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 p-2">
      ...
      <div>
        ...
        <p className="text-[var(--text-muted)] font-medium">All appointments across your hospital's doctors.</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total', value: stats.total, color: 'from-violet-500 to-indigo-600', icon: <Calendar size={20} /> },
          { label: 'Pending', value: stats.pending, color: 'from-amber-400 to-orange-500', icon: <AlertCircle size={20} /> },
          { label: 'Accepted', value: stats.accepted, color: 'from-blue-500 to-cyan-500', icon: <CheckCircle size={20} /> },
          { label: 'Completed', value: stats.completed, color: 'from-emerald-500 to-green-600', icon: <CheckCircle size={20} /> },
        ].map((s, i) => (
          <motion.div
            key={s.label}
      ...
      <div className="card p-0 bg-[var(--bg-card)] border-[var(--border-main)] shadow-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-20 text-center text-[var(--text-muted)]">
            ...
            <p className="text-sm mt-1">Appointments will appear here once patients book with your doctors.</p>
          </div>
        ) : (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 flex items-center justify-center font-bold text-sm">
                            {apt.patient?.name?.[0]?.toUpperCase() || <User size={14} />}
                          </div>
                          <div>
                            <p className="font-bold text-sm text-[var(--text-main)]">{apt.patient?.name || 'Unknown'}</p>
                            <p className="text-xs text-[var(--text-muted)]">{apt.patient?.gender || '—'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm text-[var(--text-muted)] max-w-[180px] truncate">{apt.reason || '—'}</td>
                      <td className="px-6 py-4">
